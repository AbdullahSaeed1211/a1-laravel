<div
    ...
    x-init="
        ...
        document.addEventListener('mouseleave', () => {
            if (document.documentElement.scrollY > 100) show = true;
        }, { once: true });
    "
    x-show="show"
    x-cloak
    @keydown.escape.window="show = false; sessionStorage.setItem('a1_lead_popup_dismissed', '1')"
    class="fixed inset-0 z-[9999] flex items-center justify-center p-4"
>
    <div class="absolute inset-0 bg-black/70 backdrop-blur-sm" @click="show = false; sessionStorage.setItem('a1_lead_popup_dismissed', '1')"></div>
    <div class="relative bg-asphaltBlack rounded-3xl shadow-2xl max-w-lg w-full p-6 md:p-8 overflow-y-auto max-h-[90vh] border border-white/10">
        <button @click="show = false; sessionStorage.setItem('a1_lead_popup_dismissed', '1')" class="absolute top-4 right-4 text-white/40 hover:text-white transition-colors z-10">
